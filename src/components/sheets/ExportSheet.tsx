...
interface Props {
  onRender: (name: string, sampleRate: number, onProgress: (p: number) => void) => Promise<{ blob: Blob; filename: string }>;
  onClose: () => void;
}
...
export default function ExportSheet({ onRender, onClose }: Props) {
  ...
  async function handleCreate() {
    ...
    setProgress(0);
    try {
      const res = await onRender(name.trim() || 'Mon morceau', sampleRate, setProgress);
      setResult(res);
      const file = new File([res.blob], res.filename, { type: 'audio/wav' });
        ...
        <>
          ...
          <div className="sheet-field">
            ...
            <div className="chip-row">
              <button className="chip chip-selected" disabled>WAV</button>
            </div>
          </div>
